...
export default async function ParticipantCompetitionsPage() {
  ...
  const { data: participations, error: participationsError } = await client.from("tournament_participants").select("id,tournament_id,display_name_snapshot,final_rank,status").eq("user_id", auth.user.id);
  if (participationsError) throw fromPostgrestError(participationsError);
  ...
  if (accessResult?.error) throw fromPostgrestError(accessResult.error);
  const tournamentIds = (participations ?? []).map((item) => item.tournament_id);
  const [tournamentsResult, rankingsResult, badgesResult] = await Promise.all([
  ...
  const resultError = tournamentsResult.error ?? rankingsResult.error ?? badgesResult.error;
  if (resultError) throw fromPostgrestError(resultError);
  ...
  return <main className="mx-auto min-h-screen max-w-lg space-y-5 bg-slate-950 p-4 text-white"><header className="flex items-center justify-between"><div><h1 className="text-3xl font-bold text-cyan-300">Mes compétitions</h1><p className="text-sm text-gray-400">Planning, combats et résultats</p></div><Link href="/archives" className="text-cyan-300">Archives</Link></header>
    ...
    <section className="rounded-xl border border-cyan-400/30 bg-black/50 p-4"><h2 className="mb-2 font-bold text-cyan-300">Statistiques et résultats</h2>{!rankings?.length && <p className="text-sm text-gray-400">Aucun résultat enregistré.</p>}{(rankings ?? []).map((ranking) => <p key={ranking.mode ?? "overall"} className="text-sm">{ranking.mode ?? "Général"} : {ranking.score} pts · {ranking.victories} V · {ranking.defeats} D · {ranking.draws} N · {ranking.matches_played} matchs</p>)}</section>
